...
function Waveform(
  ...
  const scaleLinearHeight = scaleLinear().domain([0, height]).range([0, height]);
  const chunks = waveform //_.chunk(waveform.samples, waveform.width / ((width - 60) / 3));
  return (
    <View style={[{
      height,
      width,
      ...
      {chunks.map((chunk, i) => (
        ...
        >
          <View style={{
            ...
            width: 2,
            marginRight: 1,
            height: chunk //scaleLinearHeight(mean(chunk)),
          }}
          />
        </TouchableOpacity>
      ))}
